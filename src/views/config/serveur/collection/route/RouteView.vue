...
import { ref } from "vue";
import { columnsRoute } from '@/utils/kendo/col';
...
const collections = ref<Array<any>>([{name:"user"},{name:"personnel"},{name:"stock"},{name:"caisse"}]);
const methodHttp = ref("");
const methodsHttp = ref<Array<any>>([{name:"POST"},{name:"GET"},{name:"PUT"},{name:"DELETE"}]);
    ...
    >
      <template v-slot:activator="{ props }">
        <v-btn color="secondary" dark v-bind="props" rounded="outlined" class="ml-auto mt-5">
            <v-icon class="mr-2">mdi-account-multiple-plus</v-icon>Ajouter une route
        </v-btn>
      </template>
      <v-card>
        <v-toolbar dark style="flex: unset">
          ...
          <v-toolbar-title class="text-white">Append Routes</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
        ...
        <v-card-item>
            <Form @submit="">
                <v-row>
                    ...
                    <v-autocomplete v-model="server"   item-title="name" item-value="name"   :items="servers" color="primary" variant="outlined" hide-details></v-autocomplete>
                    </v-col>
                    <v-col cols="12" sm="4">
                        <v-label class="text-subtitle-1 font-weight-medium pb-2">Collection*</v-label>
                        <v-autocomplete v-model="collection" label="Collection"  item-title="name" item-value="name"   :items="collections" color="primary" variant="outlined" hide-details></v-autocomplete>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="12" sm="4">
                    <v-label class="text-subtitle-1 font-weight-medium pb-2">Methode</v-label>
                    <v-autocomplete v-model="methodHttp" item-title="name" item-value="name"  :items="methodsHttp" color="primary" variant="outlined" hide-details></v-autocomplete>
                    </v-col>
                    <v-col cols="12" sm="4">
                        <v-label class="text-subtitle-1 font-weight-medium pb-2">Route*</v-label>
                        <VTextField
